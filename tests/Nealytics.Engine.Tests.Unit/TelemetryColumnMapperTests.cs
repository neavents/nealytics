...
{
    private static (Guid[], string[], string[], string[], string?[], string[], string?[], string[], DateTimeOffset[]) Alloc(int n)
        => (new Guid[n], new string[n], new string[n], new string[n], new string?[n], new string[n], new string?[n], new string[n], new DateTimeOffset[n]);
    ...
    public void Fill_MapsEveryColumn_IncludingNullItemIdAndUtcTimestamp()
    {
        ...
        var (eventIds, projectIds, tenantIds, sessionIds, userIds, eventTypes, itemIds, metadataJsons, timestamps) = Alloc(2);

        TelemetryColumnMapper.Fill(batch, 2, eventIds, projectIds, tenantIds, sessionIds, userIds, eventTypes, itemIds, metadataJsons, timestamps);

        eventIds[0].Should().Be(id0);
        projectIds[0].Should().Be("p0");
        tenantIds[1].Should().Be("t1");
        sessionIds[0].Should().Be("s0");
        userIds[0].Should().Be("u0");
        userIds[1].Should().BeNull("an anonymous event carries a NULL user_id");
    ...
    public void Fill_RespectsCount_AndIgnoresTrailingSlots()
    {
        ...
        var (eventIds, projectIds, tenantIds, sessionIds, userIds, eventTypes, itemIds, metadataJsons, timestamps) = Alloc(4);

        TelemetryColumnMapper.Fill(batch, 1, eventIds, projectIds, tenantIds, sessionIds, userIds, eventTypes, itemIds, metadataJsons, timestamps);

        projectIds[0].Should().Be("only");
        projectIds[1].Should().BeNull("only the first slot should be populated");
        userIds[0].Should().BeNull("the single event has no user_id");
    }

    [Fact]
    public void BuildColumns_ContainsAllNineColumns_SlicedToCount()
    {
        var (eventIds, projectIds, tenantIds, sessionIds, userIds, eventTypes, itemIds, metadataJsons, timestamps) = Alloc(10);

        Dictionary<string, object?> columns = TelemetryColumnMapper.BuildColumns(
            3, eventIds, projectIds, tenantIds, sessionIds, userIds, eventTypes, itemIds, metadataJsons, timestamps);
        ...
        ((ArraySegment<string?>)columns["user_id"]!).Count.Should().Be(3);
